        ...
        <div class="row mb-2">
            ...
            <div class="col-9">
                ...
                <a href="{{ route('dishes.index', ['filter' => 'trashed']) }}" class="button-menu text-decoration-none ibm-plex-mono-regular position-relative">
                    ...
                    @if ($trashedCount > 0)
                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger ibm-plex-mono-regular">{{ $trashedCount }}</span>
                    @endif
                </a>
            </div>
            ...
            <div class="col-3 d-flex justify-content-end">
                <a href="{{ route('dishes.create') }}" class="button-menu text-decoration-none ibm-plex-mono-regular text-center mb-2">
                    Aggiungi Piatto
                </a>
            </div>
                        ...
                        <div class=" row w-100 d-flex">
                            <div class="col-12 justify-content-center position-relative restaurant-image p-0">
                                ...
                                @if (filter_var($restaurant->image, FILTER_VALIDATE_URL))
                                        <img src="{{ $restaurant->image }}" class="card-img-top w-100" alt="{{ $restaurant->name }}">
                                        ...
                                        <div class="d-flex justify-content-end">
                                            @if ($dish->trashed())
                                                ...
                                                <form action="{{ route('dishes.restore', ['id' => $dish->id]) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    <button type="submit" class="button-menu py-2  me-2">Ripristina</button>
                                                </form>
                                                ...
                                                <form onsubmit="return confirm('Sei sicuro di voler eliminare DEFINITIVAMENTE questo piatto?')" action="{{ route('dishes.forceDestroy', $dish->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger py-2  me-2">Elimina</button>
                                                </form>
                                            @else
                                                ...
                                                <form onsubmit="return confirm('Attenzione! Stai cancellando questo elemento, vuoi continuare?')" action="{{ route('dishes.destroy', $dish->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="button-delete  position-absolute top-0 end-0">
